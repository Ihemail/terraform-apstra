...
locals {
  ...
  switches = {
    ...
    vex_esi_001_leaf1    = { management_ip = "10.206.224.21", device_key = "54040ACEE015", initial_interface_map_id = "Juniper_vQFX__AOS-7x10-Leaf", hostname = "vex-esi-001-leaf1" }
    vex_esi_001_leaf2    = { management_ip = "10.206.209.152", device_key = "54040ACED198", initial_interface_map_id = "Juniper_vQFX__AOS-7x10-Leaf", hostname = "vex-esi-001-leaf2" }
    vex_std_001_leaf1    = { management_ip = "10.206.206.179", device_key = "54040ACECEB3", initial_interface_map_id = "Juniper_vQFX__AOS-7x10-Leaf", hostname = "vex-std-001-leaf1" }
...
resource "null_resource" "shell_script" {
  ...
  provisioner "local-exec" {
    command = "scp start.vex.exp root@centos1:${each.value.hostname}.exp"
  }
  provisioner "local-exec" {
  ...
  #}
  #provisioner "file" {
  #  source  = "${each.value.hostname}.sh"
  #  destination  = "/root/terraform/${each.value.hostname}.sh"
  #}
  #provisioner "file" {
  #  source  = "start.exp"
  #  destination  = "/root/terraform/start.exp"
  #}  
}
