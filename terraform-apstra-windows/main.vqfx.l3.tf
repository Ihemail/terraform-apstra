...
provider "apstra" {
  ...
  url = "https://admin:AOSserver%401024@127.0.0.1:9800"        # required: WebUi: https://127.0.0.1:9800/
  tls_validation_disabled = true         # optional
  blueprint_mutex_disabled = true        # Don't attempt worry about competing clients
  # cmd_prompt(windows)> ssh -NL 9800:10.220.29.233:443 ihazra@10.32.192.47
...
locals {
  ...
  switches = {
    spine1             = { management_ip = "10.220.29.50", device_key = "000000AAAAAA", initial_interface_map_id = "Juniper_vQFX__AOS-7x10-Spine", hostname = "spine1" }
    spine2             = { management_ip = "10.220.2.200", device_key = "000000AABBBB", initial_interface_map_id = "Juniper_vQFX__AOS-7x10-Spine", hostname = "spine2" }
    vqfx_esi_001_leaf1 = { management_ip = "10.220.11.76", device_key = "000000AA1111", initial_interface_map_id = "Juniper_vQFX__AOS-7x10-Leaf", hostname = "vqfx-esi-001-leaf1" }
    vqfx_esi_001_leaf2 = { management_ip = "10.220.2.224", device_key = "000000AA2222", initial_interface_map_id = "Juniper_vQFX__AOS-7x10-Leaf", hostname = "vqfx-esi-001-leaf2" }
    vqfx_std_001_leaf1 = { management_ip = "10.220.3.84", device_key = "000000AA3333", initial_interface_map_id = "Juniper_vQFX__AOS-7x10-Leaf", hostname = "vqfx-std-001-leaf1" }
...
resource "null_resource" "shell_script" {
  ...
  command = "echo #!/usr/bin/bash >> ${each.value.hostname}.sh && echo cp start.exp ${each.value.hostname}.exp >> ${each.value.hostname}.sh && echo sed -i 's/mgmt__ip/${each.value.management_ip}/g' ${each.value.hostname}.exp >> ${each.value.hostname}.sh && echo sed -i 's/host__name/${each.value.hostname}/g' ${each.value.hostname}.exp >> ${each.value.hostname}.sh && echo expect ${each.value.hostname}.exp >> ${each.value.hostname}.sh && ping 127.0.0.1"
  }
  provisioner "local-exec" {
    command = "scp start.vqfx.exp root@centos1:${each.value.hostname}.exp"
  }
  provisioner "local-exec" {
    command = "scp ${each.value.hostname}.sh root@centos1:${each.value.hostname}.sh"
  }
}
...
resource "null_resource" "hostname_change_with_options" {
  ...
  for_each   = local.switches
  provisioner "local-exec" {
    command = "ssh root@centos1 chmod +x ${each.value.hostname}.exp && ssh root@centos1 chmod +x ${each.value.hostname}.sh && ssh root@centos1 sh ${each.value.hostname}.sh"
  }
}
...
resource "null_resource" "delete_hostname_scripts" {
  ...
  for_each         = local.switches
  provisioner "local-exec" {
    command = "del ${each.value.hostname}.sh"
  }
  provisioner "local-exec" {
    command = "ssh root@centos1 rm -rf ${each.value.hostname}.*"
  }
  #provisioner "local-exec" {
